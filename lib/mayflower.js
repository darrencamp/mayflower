...
function Mayflower (connectionObject, migrateDirectory, migrationsTable)
{
	...
	{
		console.log(_config);
		var db = new MsSql.ConnectionPool(_config);
	...
	/**
	 * @param callback {function(?Error, Script[])}
	 */
	this.getOutstandingMigrations = function (callback)
	{
		var _this = this;
		var db;
		var outstanding = [];
		...
		Athena.waterfall(
			[
				...
				function (cb, d) {
					...
					request.query('select * from [' + migrationsTable + ']', cb);
				},
				function (cb, rows)
				{
                    console.log('getOutstandingMigrations:select * from migrations');
                    console.log(rows);
					try
					{
						...
						{
							hash = scripts[i].hash;
							for (var x = 0; x < (rows.length || rows.recordset.length); x++)
							{
								if (hash === rows[x].Hash)
									continue scripts_loop;
							}
							
							outstanding.push(scripts[i]);
						}
			...
			function (hlog)
			{
				...
				callback(hlog.failed ? hlog : null, outstanding);
			}
		);
	};
	...
	this.migrate = function (db, options, script, callback)
	{
		...
		Athena.waterfall(
			[
				...
				function (cb)
				{
                    console.log(`tables exists ${_tableExists}`);
				...
				{
                    var request;
                    console.log('migrate:select top 1 * from [migrations] where [Hash] = @hash');
                    console.log(rows);
					if ((rows.length || rows.recordset.length) > 0)
					{
						// migration has already been applied
						var r = (rows[0] || rows.recordset[0]);
				...
				function (cb, rows)
				{
                    console.log('migrate:select top 1 * from [migrations] where [Filename] = @script');
                    console.log(rows);
					if ((rows.length || rows.recordset.length) > 0)
					{
						if (!options.force)
            ...
            if (parts[0] && parts[1]) {
                switch (parts[0].toUpperCase())
                {
                    ...
                    case 'TDS_VERSION':
                        ...
                        break;
                    case 'ENCRYPT':
                        result.options.encrypt = parts[1].toUpperCase() === "TRUE" || parts[1].toUpperCase() === "YES";
                        break;
                    case 'TRUSTEDCONNECTION':
                        result.options.trustedconnection = parts[1].toUpperCase() === "TRUE" || parts[1].toUpperCase() === "YES";
                        break
                    case 'TRUSTED_CONNECTION':
                        result.options.trusted_connection = parts[1].toUpperCase() === "TRUE" || parts[1].toUpperCase() === "YES";
                        break
                    default:
                        result.options[parts[0].toLowerCase()] = parts[1];
                }
            }
        }
console.log(result);
